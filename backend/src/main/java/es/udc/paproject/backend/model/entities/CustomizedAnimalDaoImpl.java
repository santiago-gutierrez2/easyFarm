...
public class CustomizedAnimalDaoImpl implements CustomizedAnimalDao{
    ...
    @Override
    public Slice<Animal> find(Long farmId, String name, Long identificationNumber, String startDate, String endDate, Boolean isMale, int page, int size) {

        String[] tokensName = getTokens(name);
        String queryString = "SELECT a from Animal a " +
                "WHERE a.belongsTo.id = " + farmId.toString();

        if (tokensName.length > 0 || startDate != null || endDate != null || identificationNumber != null || isMale != null) {
            queryString += " AND ";
        }
